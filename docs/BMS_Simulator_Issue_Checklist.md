...
- [ ] ⚠️ **Bulk/multi-select "acknowledge all" feature may contradict real BMS behavior.** Lev explicitly confirmed the real system requires acknowledging *every* alarm individually — there's no one-click "acknowledge all," specifically so lazy engineers can't skip past active alarms. Katherine has since built a multi-select/select-all acknowledge feature into the simulator. **This needs to be reconciled with Lev** — either removed, or reframed as an intentional simplification for students (not left as an unexamined conflict).
- [ ] ⬜ **Some alarms can't be acknowledged at all** — bug identified in the most recent pre-launch test session; root cause not yet found.
...
Remaining big open buckets, unchanged: (1) manual weather control — Lev's most repeated ask, still not built; (2) the alarm-acknowledgment feature conflict that needs a decision with Lev; (3) finishing the modal/description pop-ups and instructor/student view split.
